...
Use `npm install` to install dependencies. `npm run dev` starts TelePi with `tsx` against `src/index.ts` for local development. `npm run check` runs a no-emit TypeScript check, with `npm run lint` as an alias. `npm run build` compiles TypeScript into `dist/`. `npm start` runs the built app from `dist/index.js`. `npm test` runs the Vitest suite once, and `npm run test:coverage` enforces coverage thresholds. For Docker, use `docker compose up --build` after creating `.env`.
